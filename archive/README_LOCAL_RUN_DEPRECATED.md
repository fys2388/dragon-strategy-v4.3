# ⛔ 已废弃 — 本机定时运行配置（2026-09-17 归档）

> **不要按本文档执行。** 本文档描述的「本机为主、盘中每 5 分钟定时推送」架构已废弃，
> 与现行架构直接冲突（`AGENTS.md` 铁律 1「部署只在云端」；唯一自动触发源是 Cloudflare Worker）。
> 照它注册 Windows 计划任务会造成 **云端 + 本机双份推送**。

归档原因：
1. 现行架构是 Cloudflare Worker 每 15 分钟调 `workflow_dispatch`、GitHub Actions 每 30 分钟
   执行一次完整扫描（见 `docs/HANDOFF.md` §3 与 `README.md`「部署与推送链路」）。
2. 推送文案已在提交 `1fdc849` 精简为「极简版」，本文档「验证」章节引用的诊断行格式
   （`📡 数据源：… | 校验：… | 市场环境：…`）已不存在。
3. `scripts/run_local.ps1` / `run_local.sh` 仍可用，但仅限**手动跑一次做链路验证**，
   不再用于定时推送；且本机有代理问题，不能用本地扫描评估性能。

保留原文仅为追溯当时的设计意图，不作为操作依据。
